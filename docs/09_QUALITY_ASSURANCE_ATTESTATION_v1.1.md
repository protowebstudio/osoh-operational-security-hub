...
### Frontend Symmetry Verification (TFM MVP)

* Strict TypeScript enabled and build passes (`npm run build`).

* Frontend Docker multi-stage build succeeds and serves static assets via nginx.

* Frontend service layer enforced (no component-level fetch usage).

* Authentication flow implemented via AuthService with token hydration.

* Global 401 handling enforced (token cleared + redirect to `/`).

* Dashboard loads Sites and RiskSnapshot via typed services and DTOs.

* Deterministic health endpoint validated:

  * GET /api/health → { "status": "ok", "timestamp": "<iso8601>" }

* Integration proof:

  * Frontend reachable at [http://localhost:3000](http://localhost:3000)
  * Backend health reachable at [http://localhost:8000/api/health](http://localhost:8000/api/health)
